...
import viewSummary from '../../viewSummary/[pageid]/page';
...
function History() {
  ...
  return (
    <div className='bg-gray-200 min-h-[150px] p-5 rounded-xl'>
        <h2 className='font-bold text-xl md:text-2xl mb-2 '>Your Previous Lectures</h2>
        {list?.length==0&&<h2 className='text-gray-500'>You don't have any previous Lectures</h2>}
        <div>
          {list.map((item,index)=>(item.coachingOption=='Lecture on Topic'||item.coachingOption=='Meditation'||item.coachingOption=='Language skill')&&
          (
              <div key={index} className='mt-2 border-2 border-neutral-400 rounded-2xl group p-2 px-4 hover:scale-105 transition-all duration-300 flex items-center justify-between'>
                ...
                <Button className={"hover:cursor-pointer invisible group-hover:visible"} variant="outline">View Notes</Button>
                </Link>
              </div>
          ))}     
          </div>   
