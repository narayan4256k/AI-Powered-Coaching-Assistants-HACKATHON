...
export default function HeroSectionOne() {
  return (
    <div
      className="relative flex max-w-full mx-2 md:mx-5 flex-col items-center justify-center">
      <Navbar />
      <div
        className="absolute inset-y-0 left-0 h-full w-px bg-neutral-200/80 dark:bg-neutral-800/80">
        <div
          className="absolute top-0 h-40 w-px bg-gradient-to-b from-transparent" />
      </div>
      <div
        className="absolute inset-y-0 right-0 h-full w-px bg-neutral-200/80 dark:bg-neutral-800/80">
        <div
          className="absolute h-40 w-px bg-gradient-to-b from-transparent " />
      </div>
      <div
        className="absolute inset-x-0 bottom-0 h-px w-full bg-neutral-200/80 dark:bg-neutral-800/80">
        <div
          className="absolute mx-auto h-px w-40 bg-gradient-to-r from-transparent " />
      </div>
      <div className="px-4 py-10 md:py-20">
        <h1
          ...
          {"We Are Yet working on our website"
            .split(" ")
            .map((word, index) => (
              <motion.span
                key={index}
        ...
        <motion.p
          ...
          Visit website we have finished creating....
        </motion.p>
        <motion.div
          initial={{
            opacity: 0,
  ...
  return (
    <nav
      ...
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-base font-bold md:text-2xl">VocalCoach AI</h1>
      </div>
      <Link href={"/dashboard"}>
      <button
